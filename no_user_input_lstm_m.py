...
class Encoder:
    ...
    def __init__(self, n_x, x_dim, n_io, n_time, learning_rate,
                 dict_filename, Save_Init_Weights, Testing):
...
class Enc_Dec:
    ...
    def __init__(self, x_dim, n_io, n_time, learning_rate, 
                 ...
                 num_samples, Save_Init_Weights, Testing):
        self.x_dim = x_dim      
...
#        print("Densities = ", self.Dec.p.density)
        ...
        print("Total Density for sample = ", self.Dec.p.total_density)
...
def main1():
    ...
    x1 = np.random.uniform(-1, 1, (10, 3, 2))
    x2 = np.random.uniform(-1, 1, (10, 3, 2))
#    print(x1[0])
#    print(x2[0])
#    x = np.hstack((x1[0],x2[0]))
#    print(x)
    target = np.random.uniform(0, 0, (10, 3, 2))

    # Initialize batch size
    b = 3

    # Initialize LSTM
    L = Encoder(2, np.array([2, 2]), 2, 3, 0.7, 'en.vec', 30000)

    for i in range(100):
        # Reset batch
        batch = np.random.randint(0, 9, b)
        ...
        for j in batch:
            ...
            L.p.io_update(np.hstack((x1[j], x2[j])), target[j], 0)

            # Update states and errors
            L.forward()

            # Check if objective function is below tolerance
            if (L.p.totalErr < tol):
                # Print feature and target
                print("Feature = ", np.hstack((x1[j], x2[j])))
                print("Target = ", target)
